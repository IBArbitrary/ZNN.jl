ACTIVATIONS = [
    ...
    :linear, :cabslu, :modrelu, :zrelu, :crelu, :cardioid,
...
rlu(z::Number) = real(z)
ilu(z::Number) = imag(z)
_abs(z::Number) = abs(z + eps(real(eltype(z))))
abslu(z::Number) = _abs(z)
rrelu(z::Number) = relu(real(z))
irelu(z::Number) = relu(imag(z))
sigmoid_cart_comp(z::Number) = sigmoid(real(z) + imag(z))
softmax_polar_comp(z) = (softmax(_abs.(z)) + softmax(angle.(z))) / 2
softmax_abs(z) = softmax(_abs.(z))
...
# Complex valued
linear(z::Number) = z
cabslu(z::Number) = _abs(z) + 1im*zero(real(eltype(z)))

## ReLU based
modrelu(z::Number, b::AbstractFloat=1.0) = relu(_abs(z) + b) * z / _abs(z)
...
## Polar form
pol_tanh(z::Number) = tanh(_abs(z)) * exp(1im * angle(z))
pol_sigmoid(z::Number) = sigmoid(_abs(z)) * exp(1im * angle(z))
pol_selu(z::Number, α::AbstractFloat=1.0) = selu(_abs(z), α) * exp(1im * angle(z))

## Phasor form
georgiou(z::Number, c::AbstractFloat=1.0, r::AbstractFloat=1.0) = z / (c + _abs(z) / r)

...
for name in (:scalarmax, :complexmixednorm)
	...
	@eval function $((Symbol("$(name)pool_direct!")))(
		...
		p = if $(name != :complexmixednorm)
			0
		else
			!haskey(kwargs, :p) && error("complexmixednormpool needs keyword argument `p`")
			kwargs[:p]
		end

		q = if $(name != :complexmixednorm)
			0
		else
			!haskey(kwargs, :q) && error("complexmixednormpool needs keyword argument `q`")
			kwargs[:q]
		end
		...
		f = if $(name != :scalarmax)
			...
			!haskey(kwargs, :f) && error("complexmixednormpool needs keyword argument `f`")
			kwargs[:f]
		end

		# Each loop, we initialize `m` to something, set that here.

		m_val = if $(name == :scalarmax)
			T_real <: AbstractFloat ? nextfloat(typemin(T_real)) : typemin(T_real)
				...
				for h in h_region
					...
					for w in w_region
						...
						for kd in 1:kernel_d,
							...
							elseif $(name == :complexmixednorm)
								# y = (∑ᵢ (|xᵢ|_q)^p)^(1 / p), here to calculate ∑ᵢ (|xᵢ|_q)^p
								xv = x[input_kw, input_kh, input_kd, c, batch_idx]
								a, b = abs(real(xv)), abs(imag(xv))
								m += (a^q + b^q)^(p/q)
							else
								error("Unimplemented codegen path")
							end
						...
						m = $(name == :complexmixednorm) ? m^(T_real(1) / p) : m
				...
				for d in d_region
					...
					for h in h_region
						...
						for w in w_region
							...
							$(name == :complexmixednorm) && (m = m^(T_real(1) / p))
							y[w, h, d, c, batch_idx] = _alpha * m # + _beta * y[w, h, d, c, batch_idx]
						end
					end
				end
